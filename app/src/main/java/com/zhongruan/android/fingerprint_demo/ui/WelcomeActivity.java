...
import com.zhongruan.android.fingerprint_demo.utils.APPUtil;
    ...
    public void initData() {
        if (Build.MODEL.equals(Utils.DEVICETYPE_YLT1) || Build.MODEL.equals(Utils.DEVICETYPE_YLT2)) {
            if (DbServices.getInstance(getBaseContext()).loadAllrzfs().size() == 0 && DbServices.getInstance(getBaseContext()).loadAllrzzt().size() == 0) {
                MyApplication.getDaoInstant(getBaseContext()).getDatabase().execSQL("INSERT INTO " + Sb_settingDao.TABLENAME + " (settingid,sb_ip,sb_ms,sb_hyfs,sb_finger_fz,sb_finger_cfcs,sb_face_xsd,sb_finger_bdfw, sb_face_cfcs)   VALUES ('1','192.168.1.1','1','0','1','0','0','0','0')");
            ...
            //启动服务
            startService(new Intent(WelcomeActivity.this, MyService.class));
            //注册广播接收器
            myReceiver = new MyReceiver();
            IntentFilter filter = new IntentFilter();
            filter.addAction("MyService");
            WelcomeActivity.this.registerReceiver(myReceiver, filter);
        } else {
